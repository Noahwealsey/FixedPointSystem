...
fp_int_t fp_mul(fp_int_t a, fp_int_t b){
	return ((fp_int_xl_t)a*b) >> FP_SCALE;
}
...
int main(){
	...
	const fp_int_t ref_voltage = fp_to_float(3.3f);
	const fp_int_t adc_max_volt = FP_PUSH_INT(4095);
	int32_t adc_reading = 2069;

	fp_int_t adc_fp = float_to_fp(adc_reading);
	fp_int_t result_voltage = fp_mul(adc_fp, fp_div(adc_reading, adc_max_volt));
	printf("resulting vooltage is  %fV\n", fp_to_float(result_voltage));
